...
function deployContract() {
  web3.eth.getTransactionCount(process.env.RINKEBY_ADDRESS).then((txnCount) => {
    ...
    const source = fs.readFileSync(__dirname+'/../contracts/OrganiCoin.sol');
    const compiled = solc.compile(source.toString(), 1);
    const bytecode = compiled.contracts[':OrganiCoin'].bytecode;
    const rawContractTx = {
...
function showABI() {
    const source = fs.readFileSync(__dirname+'/../contracts/OrganiCoin.sol');
    const compiled = solc.compile(source.toString(), 1);
    const abi = compiled.contracts[':OrganiCoin'].interface;
    console.log("abi", abi);  
}
